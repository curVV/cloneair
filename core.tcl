...
proc distance {icao_a icao_b} {
    ...
    if {[catch {set icao_a_text [exec zgrep -A 1 -i "^1 .*\s${icao_a}\s" $fgdata/Airports/apt.dat.gz]} err]} {return "Not found: $icao_a"}
    if {[catch {set icao_b_text [exec zgrep -A 1 -i "^1 .*\s${icao_b}\s" $fgdata/Airports/apt.dat.gz]} err]} {return "Not found: $icao_b"}
